...
```json
{
  ...
  "iisSettings": {
    ...
    "iisExpress": {
      "applicationUrl": "http://localhost:15156",
      "sslPort": 44315
    }
  },
  "profiles": {
    ...
    "https": {
      ...
      "dotnetRunMessages": true,
      "launchBrowser": true,
      "applicationUrl": "https://localhost:7231;http://localhost:5190",
      "environmentVariables": {
        "ASPNETCORE_ENVIRONMENT": "Development"
      }
    },
...
#### Entities
##### Презентують сутності в базі даних.
...
#### Конфігурації сутностей
##### Класи, що визначають властивості таблиць сутностей в базі даних.
...
```
#### Інтерфейс IHasId
##### Реалізує властивість Id в класах.
```C#
...
#### Models
##### Класи, які вже можуть мати логіку та використовуватимуться в самій програмі. Кожна сутність має модель з id та без нього для випадків, коли в сутності ще немає Id та коли вже є.

##### User
```C#
public class User
{
    public User(string firstName, string lastName, string email, string phoneNumber, string password, bool isAdmin, string description,
...
{
    public IdentifiedUser(int id, string firstName, string lastName, string email, string? phoneNumber, string password, bool isAdmin, string? description, int? age, string? gender, string? company) 
        : base(firstName, lastName, email, phoneNumber, password, isAdmin, description, age, gender, company)
    {
        Id = id;
    }

    public IdentifiedUser(UserEntity u) : this(u.Id, u.FirstName, u.LastName, u.Email, u.PhoneNumber, u.Password, u.IsAdmin,u.Description, u.Age, u.Gender ,u.Company)
    {
    }
...
public class Survey
{
    public Survey(string title, string? description, DateTime creationDate, DateTime? closeDate, bool isChangable, bool isActive, int ownerId)
    {
        Title = title;
        Description = description;
...
{
    public IdentifiedSurvey(int id, string title, string? description, DateTime creationDate, DateTime? closeDate, bool isChangable, bool isActive, int ownerId) 
...
#### SurveyDbContext
##### Основний клас, який предствляє сесію взаємодії з базою даних.
...
#### Repositories
##### Взаємодіють з сутностями, які записуються або витягаються в базу даних.

##### UserRepository
```C#
using Microsoft.EntityFrameworkCore;
using RESTful_API_lab.Entities;
using RESTful_API_lab.Models;
using RESTful_API_lab.ProjDbContext;

namespace RESTful_API_lab.Repositories;
...
#### Controllers
##### Описують поведінку кінцевих точок.

##### UserController
```C#
using Microsoft.AspNetCore.Mvc;
using RESTful_API_lab.Models;
using RESTful_API_lab.Repositories;

namespace RESTful_API_lab.Controllers;
